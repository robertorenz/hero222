...
class Game {
  ...
  loop(t) {
    ...
    this.acc += dt;
    try { while (this.acc >= 1000 / 60) { this.update(); this.acc -= 1000 / 60; } }
    catch (err) { console.error('update failed', err); this.acc = 0; }
    try { this.render(); } catch (err) { console.error('render failed', err); }
  }
  update() {
    this.frame++;
  ...
  killEntity(e) {
    ...
    this.explosions.push({ x: e.x + e.w / 2, y: e.y + e.h / 2, t: 14, k: 14, r: 10, soft: true });
    Sfx.hit();
  }
  placeDynamite() {
  ...
  explode(d) {
    ...
    this.explosions.push({ x: cx, y: cy, t: 26, k: 26, r: 34 });
    for (const e of this.entities) {
  ...
  kill(cause) {
    ...
    this.explosions.push({ x: this.player.x + 5, y: this.player.y + 7, t: 30, k: 30, r: 18, soft: true });
    Sfx.die();
  }
  afterDeath() {
    this.lives--;
  ...
  render() {
    ...
    ctx.save();
    try { this.renderWorld(ctx, L, ox, oy, f); } finally { ctx.restore(); }
    this.renderOverlays(ctx, L, ox, oy, f);
  }
  renderWorld(ctx, L, ox, oy, f) {
    ctx.translate(-ox, -oy);
    ...
    for (const x of this.explosions) {
      const k = x.k || 26, pr = Math.max(0, (1 - x.t / k) * x.r);
  ...
  renderOverlays(ctx, L, ox, oy, f) {
    const p = this.player, tx0 = this.sx * SW, ty0 = this.sy * SH;
    // darkness
